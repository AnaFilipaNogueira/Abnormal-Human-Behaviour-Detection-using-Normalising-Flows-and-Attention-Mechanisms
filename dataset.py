...
def gen_dataset(person_json_root, num_clips=None, kp18_format=True, ret_keys=False, ret_global_data=True, **dataset_args):
    ...
    for person_dict_fn in tqdm(json_list):
        ...
        elif dataset == 'ShanghaiTech' or dataset == 'ShanghaiTech-HR':
        ...
        #if max(clip_segs_data_np[...,2].flatten()) > 1 or min(clip_segs_data_np[...,2].flatten()) < 0:
        #    list_err.append(clip_segs_data_np[...,2].flatten())
        #    print(max(clip_segs_data_np[...,2].flatten()), min(clip_segs_data_np[...,2].flatten()))
