...
def init_sub_args(args):
    if args.dataset == "UBnormal" or args.dataset == "UBnormal-HR": 
        dataset = "UBnormal"
    elif args.dataset == "ShanghaiTech" or args.dataset == "ShanghaiTech-HR":
    ...
    if args.vid_path_train and args.vid_path_test and args.pose_path_train and args.pose_path_test:
        args.vid_path = {'train': os.path.join(args.data_dir, args.vid_path_train),
                         'test': os.path.join(args.data_dir, args.vid_path_test)}

        args.pose_path = {'train': os.path.join(args.data_dir, args.pose_path_train),
                          'test': os.path.join(args.data_dir, args.pose_path_test)}
    else:
    ...
    args.ckpt_dir = 'checkpoints_dir/' + args.dataset
...
def init_parser(default_data_dir='/nas-ctm01/datasets/public/', default_exp_dir='data/exp_dir'):
    ...
    parser.add_argument('--dataset', type=str, default='ShanghaiTech',
                        choices=['ShanghaiTech', 'ShanghaiTech-HR', 'UBnormal', 'UBnormal-HR', 'Avenue', 'Avenue-HR'], help='Dataset for Eval')
    ...
    parser.add_argument('--seg_len', type=int, default=24, metavar='SGLEN', help='Number of frames for training segment sliding window')
    ...
    parser.add_argument('--L', type=int, default=1, help='Number of STG-NF blocks')
    parser.add_argument('--R', type=float, default=3., help='Average value of the Gaussian Mixture Model')
    ...
    parser.add_argument('--n_mecatt', type=int, default=1, help='Number of attention mechanism attention in serie')
    parser.add_argument('--n_mecatt_inside', type=int, default=1, help='Number of attention mechanism attention inside the mechanism in serie')

    parser.add_argument('--network_layers', default=['res', 'gcn', 'tcn', 'relu', 'dam_att'], help='Network configuration', nargs='+')
