...
def read_convert_write(in_full_fname, out_full_fname):
    # Read results file
    if not os.path.exists(in_full_fname):
        with open(in_full_fname, 'w') as file:
            file.write('')
...
def main():
    ...
    for path, subdirs, files in os.walk(root):
        #print(path, subdirs, 'basename:', basename(path)[:-10])
        for name in files:
            run_pose = False
            ...
            if run_pose:
                # Rename constants
                if int(video_basename) < 10:
                    video_basename = video_basename[1:]
                    alphapose_orig_results_filename = args.outdir + 'alphapose-results_' + basename(path)[:-10] + video_basename + '.json'
                else:
                    alphapose_orig_results_filename = args.outdir + 'alphapose-results_' + basename(path)[:-10] + video_basename + '.json'
                
                alphapose_tracked_results_filename = args.outdir + 'processed/' + video_basename + '_alphapose_tracked_person.json'
                alphapose_results_filename = args.outdir + 'processed/' + video_basename + '_alphapose-results.json'
                print('Orig name:', alphapose_orig_results_filename)
                print('Filename:', alphapose_results_filename)
                ...
                if not os.path.exists(alphapose_orig_results_filename):
                    continue
                ...
                os.rename('alphapose-results_' + basename(path)[:-10] + video_basename + ".json", alphapose_results_filename)
