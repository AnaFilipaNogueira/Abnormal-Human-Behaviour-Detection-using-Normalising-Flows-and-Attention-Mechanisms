...
def compute_loss1(nll, reduction="mean", mean=0, weight=None):
    if reduction == "mean":
        losses = {"nll": torch.mean(nll)*weight}
    elif reduction == "logsumexp":
        losses = {"nll": torch.logsumexp(nll, dim=0)*weight}
    elif reduction == "exp":
        losses = {"nll": torch.exp(torch.mean(nll) - mean)*weight}
    elif reduction == "none":
        losses = {"nll": nll*weight}
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for itern, data_arr in enumerate(pbar):
                try:
                    ...
                    wandb.log({"losses": losses, "score": score})
